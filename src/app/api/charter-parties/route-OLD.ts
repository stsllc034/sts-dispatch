...
export async function PUT(request: Request) {
  try {
    ...
    const charterParty = await prisma.charterParty.update({
      where: {
        id: body.id,
      },
      data: {
    ...
    return NextResponse.json(charterParty);
  } catch (error) {
    console.error("Error updating charter party:", error);

    return NextResponse.json(
      { error: "Failed to update charter party." },
      { status: 500 }
    );
  }
}